...
fn impl_elm_type(input: &DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    let expanded = quote! {
        impl elm_client_gen_core::ElmType for #name {
            fn elm_type_info() -> elm_client_gen_core::ElmTypeInfo {
                elm_client_gen_core::ElmTypeInfo {
        ...
        elm_client_gen_core::inventory::submit! {
            elm_client_gen_core::ElmTypeRegistration {
                build: <#name as elm_client_gen_core::ElmType>::elm_type_info,
            }
        }
    };
...
fn build_record_kind(
    ...
    Ok(quote! {
        elm_client_gen_core::ElmTypeKind::Record {
...
fn build_enum_kind(
    ...
    for variant in variants {
        ...
        let payload_tokens = match &variant.fields {
            Fields::Unit => quote! { elm_client_gen_core::ElmVariantPayload::Unit },
            Fields::Named(named) => {
                ...
                quote! {
                    elm_client_gen_core::ElmVariantPayload::Struct(
                        vec![#(#fields_tokens),*]
                    )
                }
            }
        ...
        variant_tokens.push(quote! {
            elm_client_gen_core::ElmVariantInfo {
                rust_name: #rust_name_str,
                elm_name: #elm_name,
                json_tag: #json_tag,
    ...
    let representation_tokens = match repr {
        EnumRepr::BareString => quote! { elm_client_gen_core::EnumRepresentation::BareString },
        EnumRepr::InternallyTagged(key) => quote! {
            elm_client_gen_core::EnumRepresentation::InternallyTagged { tag_key: #key }
        },
        EnumRepr::Untagged => quote! { elm_client_gen_core::EnumRepresentation::Untagged },
    };

    Ok(quote! {
        elm_client_gen_core::ElmTypeKind::Enum {
...
fn parse_type_override(s: &str) -> (proc_macro2::TokenStream, bool) {
    ...
    if let Some(rest) = trimmed.strip_prefix("Maybe ") {
        ...
        return (
            quote! { elm_client_gen_core::ElmTypeRepr::Maybe(Box::new(#inner)) },
            true,
        );
    }
    if let Some(rest) = trimmed.strip_prefix("List ") {
        let (inner, _) = parse_type_override(rest);
        return (
            quote! { elm_client_gen_core::ElmTypeRepr::List(Box::new(#inner)) },
            false,
        );
    }
    if let Some(rest) = trimmed.strip_prefix("Dict String ") {
        let (inner, _) = parse_type_override(rest);
        return (
            quote! { elm_client_gen_core::ElmTypeRepr::Dict(Box::new(#inner)) },
            false,
        );
    }
    let primitive = match trimmed {
        "String" => Some(quote! { elm_client_gen_core::ElmTypeRepr::String }),
        "Int" => Some(quote! { elm_client_gen_core::ElmTypeRepr::Int }),
        "Float" => Some(quote! { elm_client_gen_core::ElmTypeRepr::Float }),
        "Bool" => Some(quote! { elm_client_gen_core::ElmTypeRepr::Bool }),
        "Time.Posix" => Some(quote! { elm_client_gen_core::ElmTypeRepr::Posix }),
        "Encode.Value" | "Decode.Value" | "Json.Encode.Value" | "Json.Decode.Value" => {
            Some(quote! { elm_client_gen_core::ElmTypeRepr::Value })
        }
        _ => None,
    };
...
fn rust_type_to_elm_repr(ty: &Type) -> syn::Result<(proc_macro2::TokenStream, bool)> {
    match ty {
        ...
        Type::Tuple(t) => {
            ...
            Ok((
                quote! {
                    elm_client_gen_core::ElmTypeRepr::Tuple(vec![#(#element_tokens),*])
                },
                false,
            ))
        }
        _ => Ok((
            quote! { elm_client_gen_core::ElmTypeRepr::Custom("Unknown".to_string()) },
            false,
        )),
    }
}
...
fn path_to_elm_repr(type_path: &TypePath) -> syn::Result<(proc_macro2::TokenStream, bool)> {
    ...
    match ident.as_str() {
        "String" => Ok((quote! { elm_client_gen_core::ElmTypeRepr::String }, false)),
        "bool" => Ok((quote! { elm_client_gen_core::ElmTypeRepr::Bool }, false)),
        "i8" | "i16" | "i32" | "i64" | "u8" | "u16" | "u32" | "u64" | "usize" | "isize" => {
            Ok((quote! { elm_client_gen_core::ElmTypeRepr::Int }, false))
        }
        "f32" | "f64" => Ok((quote! { elm_client_gen_core::ElmTypeRepr::Float }, false)),
        "DateTime" => Ok((quote! { elm_client_gen_core::ElmTypeRepr::Posix }, false)),
        "NaiveDate" => Ok((quote! { elm_client_gen_core::ElmTypeRepr::IsoDate }, false)),
        ...
        "Value" => Ok((quote! { elm_client_gen_core::ElmTypeRepr::Value }, false)),
        "Option" => {
            let inner = extract_single_generic_arg(segment)?;
            let (inner_tokens, _) = rust_type_to_elm_repr(&inner)?;
            Ok((
                quote! { elm_client_gen_core::ElmTypeRepr::Maybe(Box::new(#inner_tokens)) },
                true,
            ))
        }
        "Vec" => {
            ...
            let (inner_tokens, _) = rust_type_to_elm_repr(&inner)?;
            Ok((
                quote! { elm_client_gen_core::ElmTypeRepr::List(Box::new(#inner_tokens)) },
                false,
            ))
        }
        ...
        "HashMap" | "BTreeMap" => {
            ...
            Ok((
                quote! { elm_client_gen_core::ElmTypeRepr::Dict(Box::new(#value_tokens)) },
                false,
            ))
        }
        ...
        other => Ok((
            quote! { elm_client_gen_core::ElmTypeRepr::Custom(#other.to_string()) },
            false,
        )),
    }
}
...
/// `elm-client-gen-http` registry so the codegen can emit a matching
...
fn impl_elm_endpoint(args: EndpointArgs, item_fn: ItemFn) -> syn::Result<proc_macro2::TokenStream> {
        ...
        match arg {
            FnArg::Typed(pat_ty) => {
                ...
                param_dispatches.push(quote! {
                    params.push(<#ty as ::elm_client_gen_http::ElmExtractor>::elm_extractor_info());
                });
            }
    ...
    let response_dispatch = match &item_fn.sig.output {
        ReturnType::Type(_, ty) => quote! {
            <#ty as ::elm_client_gen_http::ElmResponse>::elm_response_info()
        },
        ReturnType::Default => {
            return Err(syn::Error::new(
    ...
    let registration = quote! {
        ...
        fn #builder_ident() -> ::elm_client_gen_http::ElmEndpointInfo {
            let mut params: Vec<::elm_client_gen_http::ExtractorInfo> = Vec::new();
            #(#param_dispatches)*

            ::elm_client_gen_http::ElmEndpointInfo {
                handler_name: #handler_name,
                elm_function_name: #elm_function_name,
                elm_module_path: &[#(#module_segments),*],
                method: ::elm_client_gen_http::HttpMethod::#method_variant,
        ...
        ::elm_client_gen_http::inventory::submit! {
            ::elm_client_gen_http::ElmEndpointRegistration {
                build: #builder_ident,
            }
        }
    };
